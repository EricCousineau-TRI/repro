...
class ExplicitEuler(LeafSystem):
    def __init__(self, plant, period_sec, q0):
        super().__init__()
        self.state_output_port = self.DeclareStateOutputPort()
        num_q = plant.num_positions()
        self.DeclareVectorInputPort("u", BasicVector(num_q))
        state_index = self.DeclareDiscreteState(2 * num_q)
        h = period_sec
        context = plant.CreateDefaultContext()

        def update(sys_context, discrete_state):
            u = self.get_input_port().Eval(sys_context)
            x = sys_context.get_state().get_discrete_state(state_index).get_value(0)
            ...
            discrete_state.set_value(0, x_new)

        def set_state(sys_context, q0):
            discrete_state = sys_context.get_mutable_state().get_mutable_discrete_state(state_index)
            x = discrete_state.get_value(0)
            x[:num_q] = q0
            discrete_state.set_value(0, x)
        ...
        example = plant.get_geometry_poses_output_port().Eval(context)
        cls = type(example)

        def calc_geometry(sys_context, output):
            x = sys_context.get_state().get_discrete_state(state_index).get_value(0)
            plant.SetPositionsAndVelocities(context, x)
            geometry = plant.get_geometry_poses_output_port().Eval(context)
            output.set_value(geometry)

        self.geometry_poses_output_port = self.DeclareAbstractOuputPort(
            "geometry", cls, calc_geometry
        )
