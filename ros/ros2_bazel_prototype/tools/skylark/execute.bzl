...
def execute_and_return(repo_ctx, command, additional_search_paths = [], **kargs):
    ...
    exec_result = repo_ctx.execute([program] + command[1:], **kargs)
    if exec_result.return_code == 0:
        error = None
    else:
